HPCA_decomp <- function(data,    # data.frame in long format with six labeled columns (described below)
                        ...
                        kCOV,     # knots for smoothing of covariance surfaces (scalar)
                        reControl # control argument for lme function for subject-specicic scores (list)
                        ){
#############################################################################
...
##              data: data.table used in HPCA_decomp.R (data.frame)
...
list.of.packages <- c("refund", "data.table", "fANCOVA", "mgcv", "Matrix", "caTools", "nlme", "dplyr", "rlist")
...
long_obs <- matrix(list(), nrow = length(groups))
mean_surf <- matrix(list(), nrow = length(groups))
mean_fit <- matrix(list(), nrow = length(groups))

# Calculate overall mean function within each group
for(g in 1:length(groups)){ 
  long_obs[[g]] <- NA
  for(i in 1:length(id)){
    temp = data[which(data$ID == id[i] & data$group == groups[g]), ]
    if(dim(temp)[1] > 0){
      long = as.matrix(t(unique(temp$long)))
      long = t(as.matrix(rep(long, r_tot))) 
      long_obs[[g]] = cbind(long_obs[[g]], long)
    }
  }
  long_obs[[g]] <- long_obs[[g]][-1]
  mean_surf[[g]] = fbps(matrix(data$y[which(data$group == groups[g])], nrow = f_tot), covariates = list(x = t(out1), z = long_obs[[g]]), knots = kFE, m = 2, p=3)  # Apply sandwich smoother to all data
  newdata = list(x = rep(out1, l_tot), z = rep(out2, each = f_tot))  # Form 2-D grid from longitudinal and functional domains
  mean_fit[[g]] <- matrix(predict(mean_surf[[g]], newdata = newdata)$fitted.values, nrow = f_tot)  # Predict overall mean function for 2-D grid
}


# Obtain overall mean function via pointwise average of overall group means
HPCA[[1]] = matrix(0, nrow = dim(mean_fit[[1]])[1], ncol = dim(mean_fit[[1]])[2])
for(g in 1:length(groups)){
  HPCA[[1]] = HPCA[[1]] + mean_fit[[g]]/length(groups)
}
...
for(g in groups){ 
  for(r in out3){
    ...
    reg_model = fbps(matrix(temp$y_c1, nrow = f_tot), covariates = list(x = t(out1), z = reg_long), knots = kFE, m = 2, p=3)  # Apply sandwich smoother to group-region data
...
marginal_covar <- function(data,      # data.table from preceding estimation procedures
                           ...
                           k          # knots for tensor spline basis (scalar) 
                           ){
  ...
  if(smooth == 1){  # Marginal functional or longitudinal covariance
    ...
    cov_vec_s = gam( covar ~ te(w1, w2, k = 10, bs="ps"), data = cov_vec_d)  # Smooth the pooled sample covariance
...
for(g in groups){ 
  ...
  HPCA[[3]][[g]][[1]] = marginal_covar(data, g, out1, quote(func_ind), quote(long_ind), quote(reg_ind), f_tot, l_tot, r_tot, 1, kCOV)
  # b. Calculate longitudinal marginal covariance 
  HPCA[[4]][[g]][[1]] = marginal_covar(data, g, out2, quote(long_ind), quote(func_ind), quote(reg_ind), l_tot, f_tot, r_tot, 1, kCOV)
  # c. Calculate regional marginal covariance 
  HPCA[[5]][[g]][[1]] = marginal_covar(data, g, out3, quote(reg_ind), quote(func_ind), quote(long_ind), r_tot, f_tot, l_tot, 0, NA)
}
...
eigenfunctions <- function(covariance,  # marginal covariance function
  ...
  K = length(which(cumsum(eigen_temp$values) / sum(eigen_temp$values) < .90)) + 1  # Select components that explain at least 85% FVE
...
for(g in groups){
  ...
  RE_model = lme(y_c2 ~ -1, random = RE_struct, data = data_lmm[[g]], control = reControl)  
